...
public partial class Chat : NetworkBehaviour
{
    ...
    [Header("Channels")]
    public ChannelInfo whisper = new ChannelInfo("/w", "(Do)", "(Z)", Color.magenta);
    public ChannelInfo local = new ChannelInfo("", "", "", Color.white);
    public ChannelInfo party = new ChannelInfo("/p", "(Grupa)", "(Grupa)", new Color(0.341f, 0.965f, 0.702f));
    public ChannelInfo guild = new ChannelInfo("/g", "(Gildia)", "(Gildia)", Color.cyan);
        ...
        if (!Utils.IsNullOrWhiteSpace(text))
        {
            ...
            if (text.StartsWith(whisper.command))
            {
                ...
                if (!Utils.IsNullOrWhiteSpace(user) && !Utils.IsNullOrWhiteSpace(msg))
                {
                    ...
                    else print("nie możesz szeptać do siebie");
                }
                else print("zły format: " + user + "/" + msg);
            }
